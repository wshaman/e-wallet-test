...
class jsonRPCClient
{
     ...
     * @param int $pPort
     */
    public function __construct($pUrl, $pPort=null)
    {
...
//@todo: Вернуть валидацию!
        ...
        $this->last_status  = $responseDecoded['status'];
...
// check http status code
        ...
        if (isset($this->httpErrors[$httpCode])) {
            ...
            if($r && isset($r['error'])){
                $r = $r['error'];
                throw new \RuntimeException("API answered: {$r['code']}: {$r['message']}");
            }
